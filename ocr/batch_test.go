...
func batch(t *testing.T, machine *box, pages int) Batch {
	...
	return Batch{
		...
		ID:   "alg-i-iii-0001", Images: images(t, pages),
...
func TestTheStartCommandIsWhatTheToolExpects(t *testing.T) {
	...
	for _, want := range []string{
		...
		"ocr-batch 'bourbaki-ocr/in/alg-i-iii-0001' 'bourbaki-ocr/out/alg-i-iii-0001'",
		"-j 4",
		"--rate-delay 4.5",
		"--ext png",
		"--skip-existing",
	...
	if !strings.Contains(command, `--prompt "$(cat 'bourbaki-ocr/prompt-`) {
	...
	// first makes every one of them mean bourbaki-ocr/bourbaki-ocr/..., which
	...
	if strings.Contains(command, "bourbaki-ocr/bourbaki-ocr") {
		t.Errorf("a path is doubled:\n%s", command)
	}
	for _, want := range []string{
		"'bourbaki-ocr/in/alg-i-iii-0001'",
		"'bourbaki-ocr/out/alg-i-iii-0001'",
		">'bourbaki-ocr/logs/alg-i-iii-0001.log'",
	} {
		if !strings.Contains(command, want) {
			t.Errorf("the start command has no %q in it:\n%s", want, command)
		}
	...
	for _, want := range []string{"'bourbaki-ocr/in/alg-i-iii-0001'", "'bourbaki-ocr/out/alg-i-iii-0001'", "'bourbaki-ocr/logs'"} {
...
func TestABatchIDCannotEscapeItsDirectory(t *testing.T) {
	...
	for _, id := range []string{"alg-i-iii-0001", "alg_iv_vii.2", "abc123"} {
		if err := ValidBatchID(id); err != nil {
...
func TestASummaryReadsAsOneLine(t *testing.T) {
	result := Result{Host: "server3", ID: "alg-i-iii-0001", Pages: 25, Wrote: 24,
