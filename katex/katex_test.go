...
func TestBourbakiMathematicsRenders(t *testing.T) {
	r, err := New()
	if err != nil {
		t.Fatal(err)
	}
	// Taken off the page: a tensor of a dual vector, a script letter, a
	// subscripted product, and a display with an alignment in it.
	for _, tex := range []string{
		`x^*\otimes y`,
		`\mathscr{A}_{\mathrm{red}}`,
		`\prod_{i\in I} M_i`,
