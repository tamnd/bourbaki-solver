...
//
// The three volumes in scope print their page numbers three different ways, and
// two of them are scans whose embedded text layer is somebody else's OCR:
//
...
var (
	...
	chapterOpenerRe = regexp.MustCompile(`^\s*CHAPTER\s+([IVXLCDM]+)\s*$`)
	...
	headLabelRe = regexp.MustCompile(
		`(?:^|\s)(?P<book>[A-Z])\s*[.\s]\s*(?P<ch>[IVXLCDM][IVXLCDM\s]{0,4})[.,]\s*(?P<p>\d[\d\s]{0,4})[.,]?(?:\s|$)`)
)
...
// readHeadLabel finds a page label in one running head line.
func readHeadLabel(line string) (chapter string, page int, ok bool) {
	m := headLabelRe.FindStringSubmatch(line)
	if m == nil {
		return "", 0, false
	}
	chapter = strings.Join(strings.Fields(m[2]), "")
	p, ok := readNumber(m[3])
	if !ok {
		return "", 0, false
	}
	...
	for i, pg := range pages {
		...
		case HeadLabel:
			for _, l := range headLines(pg, 2) {
				ch, p, ok := readHeadLabel(l)
				if !ok || !want[ch] {
					continue
				}
				as = append(as, anchor{pdfPage: n, chapter: ch, page: p,
					src: FromHead, raw: strings.TrimSpace(l)})
				break
			}
		case FootNumber:
			l := footLine(pg)
...
// Detect guesses the grammar by reading the volume both ways and taking the one
// that yields more anchors. The two counts are not close: the 1998 scan gives
// 695 foot numbers and no head labels at all.
...
func Build(pages []string, opt Options) (*Map, error) {
	...
	if opt.Pagination == "" {
		opt.Pagination = PerChapter
		if opt.Grammar == FootNumber {
	...
	as := readAnchors(pages, opt.Grammar, opt.Chapters)
	m := &Map{Book: opt.Book, Grammar: opt.Grammar, Pagination: opt.Pagination, PDFPages: len(pages)}

	var covers []cover
	switch opt.Pagination {
	case Continuous:
		covers = coverContinuous(as, pages, opt)
	default:
...
// coverContinuous fits one offset across the whole volume and takes the chapter
// boundaries from the chapter opener pages, which is the only exact source for
// them when the page number does not restart.
func coverContinuous(as []anchor, pages []string, opt Options) []cover {
